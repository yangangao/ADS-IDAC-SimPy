...
def get_vo_polygons(pos1,course1,speed1,pos2,course2,speed2):
    # 获得四个VO区域的多边形坐标集合
    ...
    vec_rotate_left = [vec12[0] * np.cos(gamma * np.pi / 180) - vec12[1] * np.sin(gamma * np.pi / 180),\
                       vec12[0] * np.sin(gamma * np.pi / 180) + vec12[1] * np.cos(gamma * np.pi / 180)]
    p_temp1, p_temp2 = line_circle_intersection(pp1,V_MAX * SCALE, pp2, vec_rotate_right)
    #这里还需要进一步判断
    vec1 = pp3 - pp1
    vec2 = p_temp1 - pp1
    ...
    #计算多个矩形区域
    # 1:整个速度向量区域
    point_temp = get_arc_points(pp1,pp7,pp8)
    poly_whole = np.vstack((pp1, pp7,point_temp,pp8)) #多个矩阵合并
    #python四边形对象，会自动生成逆时针的凸多边形
    poly_whole = Polygon(poly_whole).convex_hull
    
    #2：速度障碍区域
    point_temp = get_arc_points(pp1,pp5,pp6)
    poly_vo = np.vstack((pp2, pp5,point_temp,pp6)) #多个矩阵合并
    poly_vo = Polygon(poly_vo).convex_hull
    
    #3:poly_front
    point_temp = get_arc_points(pp1,pp6,pp3)
    poly_front = np.vstack((pp2, pp6,point_temp,pp3)) #多个矩阵合并
    poly_front = Polygon(poly_front).convex_hull
    
    #4:poly_rear
    point_temp = get_arc_points(pp1,pp4,pp5)
    poly_rear = np.vstack((pp2, pp4,point_temp,pp5)) #多个矩阵合并
    poly_rear = Polygon(poly_rear).convex_hull
    
    #5:poly_diverging
    point_temp = get_arc_points(pp1,pp3,pp4)
    poly_diverging = np.vstack((pp3,point_temp,pp4)) #多个矩阵合并
